...
void Renderer::saveImage(const std::string& filename) {
    ...
    for (int i = 0; i < settings.width * settings.height; ++i) {
        ...
        color = glm::pow(color, glm::vec3(1.0f / settings.gamma));
        
        // Convert to 8-bit color
        pixels[i * 3 + 0] = static_cast<unsigned char>(glm::clamp(color.r * 255.0f, 0.0f, 255.0f));
        pixels[i * 3 + 1] = static_cast<unsigned char>(glm::clamp(color.g * 255.0f, 0.0f, 255.0f));
        pixels[i * 3 + 2] = static_cast<unsigned char>(glm::clamp(color.b * 255.0f, 0.0f, 255.0f));
    }
